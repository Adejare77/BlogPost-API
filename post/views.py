...
from core.permissions import IsAdminOrSelf, AllowAnyForGetRequireAuthForWrite, IsAuthenticated
...
def post_detail(request: Request, post_id):
    if request.method == 'GET':
        try:
            ...
            post = (
                Post.objects
                .prefetch_related(
                    Prefetch('comments', queryset=top_comment_qs, to_attr='top_comments')
                )
                .annotate(
            ...
            if post.is_published == False and not request.user.is_staff:
    ...
    if request.method == 'PATCH':
        serializer = PostDetailSerializer(instance=post, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_200_OK)
