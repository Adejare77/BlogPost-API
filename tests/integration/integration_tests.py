...
class BlogAPIIntegrationTest(APITestCase):
    ...
    def test_register_user_success(self):
        """POST register -> 201."""
        url = reverse('sign-up')
        data = {'full_name': 'example', 'email': 'example@gmail.com', 'password': 'example'}
    ...
    def test_create_post_with_auth_returns_201(self):
        """ POST create -> 201 """
        url = reverse('posts')
        data = {'title': 'An introduction Post', 'content': 'This is the first Introduction to Quantum Mechanics'}
        self.client.force_authenticate(user=self.user2)
        response = self.client.post(path=url, data=data, format='json')
    ...
    def test_get_comment_replies_returns_top_three_and_reply_count(self):
        ...
        Comment.objects.create(parent=self.comment, content="reply 1", author=self.user2, post=self.post)
        Comment.objects.create(parent=self.comment, content="reply 2", author=self.user1, post=self.post)
        Comment.objects.create(parent=self.comment, content="reply 3", author=self.user1, post=self.post)
