...
class BlogAPIIntegrationTest(APITestCase):
    ...
    def test_get_own_profile_requires_auth_and_returns_profile(self):
        ...
        # anonymous user -> 401
        url = reverse("v1:user-profile", args=[self.user1.id])
    ...
    def test_admin_can_disable_user_account(self):
        ...
        url = reverse("v1:disable-account", args=[self.user2.id])
    ...
    def test_retrieve_post_public_view(self):
        # Anonymous user -> 200
        url = reverse("v1:post-detail", args=[self.post.id])
        response = self.client.get(path=url)
    ...
    def test_get_user_posts_returns_only_user_posts(self):
        """GET posts -> 200"""
        # anonymous users
        url = reverse("v1:posts", query={"author": "me"})
    ...
    def test_get_drafts_only_for_owner_or_admin(self):
        ...
        # anonymous user
        url = reverse("v1:posts", query={"status": "draft", "author": "me"})
        response = self.client.get(url)
        ...
        url = reverse("v1:posts", query={"status": "draft", "author": "ABC"})
        self.client.force_authenticate(user=self.user2)
        response = self.client.get(url)
    ...
    def test_comment_on_post_with_auth_creates_comment(self):
        ...
        url = reverse("v1:comments", args=[self.post.id])
        response = self.client.post(path=url, data=data, format="json")
    ...
    def test_update_own_comment_succeeds(self):
        ...
        url = reverse("v1:comment-detail", args=[self.comment.id])
    ...
    def test_get_replies_returns_excerpt_and_likes(self):
        """GET comment replies -> 200"""
        url = reverse("v1:replies", args=[self.comment.id])
        response = self.client.get(path=url)
