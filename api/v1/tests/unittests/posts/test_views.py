...
class PostListViewTests(APITestCase):
    ...
    def test_authenticated_user_get_others_drafts(self):
        ...
        url = reverse("v1:posts") + "?status=draft&author=CDE"
        request = self.factory.get(url)
        force_authenticate(request=request, user=self.user)
        response = views.post_list(request)
    ...
    def test_auth_user_can_get_both_own_draft_and_published_posts(self):
        ...
        """
        url = reverse("v1:posts") + "?status=all"
        request = self.factory.get(url)
        force_authenticate(request=request, user=self.user)
        response = views.post_list(request)
    ...
    def test_author_param_for_drafts(self, test_name, input_data):
        ...
        base = reverse("v1:posts") + "?status=draft"
        if not input_data:
            url = base
        else:
...
class PostDetailViewTests(APITestCase):
    ...
    def test_get_published_post_by_id_without_auth(self):
        ...
        url = reverse("v1:post-detail", args=[self.post1.id])
        request = self.factory.get(url)
    ...
    def test_get_draft_without_auth(self):
        ...
        url = reverse("v1:post-detail", args=[self.post2.id])
        request = self.factory.get(url)
    ...
    def test_deleting_post_by_id_without_auth(self):
        ...
        url = reverse("v1:post-detail", kwargs={"post_id": self.post2.id})
        request = self.factory.delete(url)
        force_authenticate(request=request, user=self.user1)
        response = views.post_detail(request, self.post2.id)
    ...
    def test_admin_can_delete_staff_posts(self):
        ...
        url = reverse("v1:post-detail", args=[self.post4.id])
        request = self.factory.delete(url)
        force_authenticate(request=request, user=self.admin1)
