...
class TestPostCreateSerializer:
    ...
    def test_serialization_includes_expected_fields(self, users):
        ...
        expected_field = {
            'id', 'author', 'content', 'title', 'is_published', 'created_at'
        }
...
class TestPostListSerializer:
    def test_serialization_includes_expected_fields(self, db):
        expected_fields = {
            'id', 'author', 'title', 'excerpt', 'likes', 'comment_count',
    ...
    def test_serialization_fields_are_read_only(self):
        ...
        assert(field.read_only for field in ser.fields.values())
...
class TestPostDetailSerializer:
    ...
    def test_read_only_fields(self, posts):
        ...
        post.like_count = 293
        post.top_comments = []
        post.comment_count = 32

        read_only_fields = {
            'id', 'author', 'likes', 'comment_count',
            'top_comments', 'created_at'
        }

        ser = PostDetailSerializer(instance=post)

        assert read_only_fields == set(ser.Meta.read_only_fields)
    ...
    def test_read_only_fields_cannot_be_updated(self, posts):
        ...
        data = {
            'id': '898393', 'author': user, 'created_at': '05-01-2026',
            'likes': 23, 'top_comments': [], 'comment_count': 12
            }
    ...
    def test_likes_maps_to_like_count(self, posts):
        post = posts['post_2']
        likes = post.like_count = 932

        ser = PostDetailSerializer(instance=post)

        assert ser.data['likes'] == likes
